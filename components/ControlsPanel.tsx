...
const SlidersPanel: React.FC = () => {
    ...
    return (
      <div className="space-y-4">
          {sliders.length > 0 ? (
              sliders.map((slider) => (
                // Use variableName as string for React key
                <div key={String(slider.variableName)} className="space-y-3">
    ...
    return (
        <div className="space-y-4">
            ...
            {sliders.map(slider => {
                ...
                return (
                    // Explicitly cast variableName as string for React key
                    <div key={String(slider.variableName)} className={`p-3 rounded-lg transition-colors ${isEnabled ? 'bg-gray-800/70' : 'bg-gray-800/30'}`}>
                        <div className="flex justify-between items-center">
            ...
            <div>
                ...
                <div className="space-y-2">
                    {inversionControls.map(control => (
                        <ToggleSwitch
                            // Cast keyof ControlConfig to string for React key
